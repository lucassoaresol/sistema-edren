...
- [ ] TASK-FE-API-002: Mover query keys de catálogo/configuração para arquivos claros por domínio e atualizar páginas/componentes consumidores.
  - Requisitos: REQ-FE-API-003, REQ-FE-API-007.
  - Verificação: `npm run typecheck`; `/products` e `/configuracoes` seguem usando as mesmas chaves lógicas.
...
- [ ] TASK-FE-API-004: Extrair padrões reutilizados da tela de configurações para componentes de domínio sem alterar visual ou comportamento.
  ...
  - Verificação: `npm run typecheck`; checagem manual da tela de configurações.
